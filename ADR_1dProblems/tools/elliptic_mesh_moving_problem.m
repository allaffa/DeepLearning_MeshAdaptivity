function [A,rhs] = elliptic_mesh_moving_problem ( a, aprime, x )
...
%
%    Input, function B ( X ), evaluates b(x);
%
...
%
  for i = 2 : n - 1
    ...
    if( bm < 0 )
        A(i,i)   = A(i,i)   - bm/( x(i+1) - x(i) );
        A(i,i+1) = A(i,i+1) + bm/( x(i+1) - x(i) );
    else
        A(i,i)   = A(i,i)   + bm/( x(i) - x(i-1) );
        A(i,i-1) = A(i,i-1) - bm/( x(i) - x(i-1) );
    end
