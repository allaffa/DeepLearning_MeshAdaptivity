function res = FV_compWise_WENO5LF1d_nonuniform(gamma,lambda,dt,q,DX)
    ...
    LF(:,I) = 0.5*(Flux(hn, gamma)+Flux(hp, gamma)-abs(lambda).*(hp-hn)); % Lax friedrichs flux
    % for j = I % for all faces of the domain cells
    %     res(:, j ) = res(:, j ) + LF(:,j)/dx;
    %     res(:,j+1) = res(:,j+1) - LF(:,j)/dx;
    % end % or alternatively:
    res(:,I) = (LF(:,I)-LF(:,I-1))./DX(I)'; % L = -df(q)/dx.
